...
export TMUXCONF=$(echo $SKEL_LINK_DIR/.tmux.conf)
...
apt-get -y install python3-venv python3-pip libssl-dev libffi-dev

# downgrade gdb to avoid bug in gdb 8.1-0ubuntu3.1 when debugging 32 bit binaries on a 64 bit system
...
# download levels
wget https://github.com/RPISEC/MBE/releases/download/v1.1_release/MBE_release.tar.gz
tar -zxvf MBE_release.tar.gz
cp -r MBE_release/levels .

# Create main user
useradd -m -s /bin/bash $REALUSER
echo "gameadmin:gameadmin" | chpasswd
...
# allow all processes to be debugged/attached to
...
LmVjG1swbSBdLS0tLS0tLS0tLS0tLS0tLS0gICAgICAgICAgDQoNCg==' | base64 -d > /etc/motd
rm /etc/legal
...
source /vagrant/external_tools.sh
...
ln -s $TMUXCONF $SKEL_DIR/.tmux.conf
...
# Some additional steps are required for getting rc.local to work on systemd
echo "[Unit]
